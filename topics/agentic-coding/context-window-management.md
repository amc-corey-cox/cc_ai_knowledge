...
Extended thinking (also called "reasoning tokens") gives the model a scratchpad for internal reasoning before producing a response. These thinking tokens are generated by the model and are visible to the user but are **not** carried forward into subsequent turns. They are ephemeral.

This distinction matters for context management:

- **Thinking tokens** are "free" in the sense that they do not accumulate across turns. Each turn's thinking starts fresh.
- **Context tokens** (conversation history, tool results) accumulate and consume the context window.
- Heavy use of extended thinking does not fill up the context window, but heavy use of verbose tool results does.
